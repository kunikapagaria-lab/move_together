...
import { Flame, Sparkles, Eye, Zap, HeartHandshake } from 'lucide-react';
...
export const Trio = () => {
  ...
  const totalGroupTasks = members.reduce((acc, m) => acc + (m.todayCompleted || 0), 0);
  const maxPossibleTasks = members.reduce((acc, m) => acc + (m.totalTasks || 8), 0);
  const synergyPercentage = maxPossibleTasks > 0 ? Math.round((totalGroupTasks / maxPossibleTasks) * 100) : 0;
  
  // Calculate average streak
  const avgStreak = members.length > 0 ? Math.round(members.reduce((acc, m) => acc + (m.streak || 0), 0) / members.length) : 0;

  return (
    <>
      <div className="w-full max-w-5xl mx-auto px-4 md:px-8 space-y-6 pb-12 h-full overflow-y-auto scrollbar-none">
        <header className="pb-2">
          <h2 className="text-2xl font-bold text-white tracking-tight mb-4">Crew Leaderboard ({group.name})</h2>
        ...
        <div className="space-y-4">
          {members.map(member => {
            const memberTasksCount = member.totalTasks || 8;
            const isSelf = member.userId._id === user?._id;
            const isFullyCompleted = member.todayCompleted >= memberTasksCount;
            const isHalfDone = member.todayCompleted >= Math.ceil(memberTasksCount / 2);
            ...
            return (
              <div key={member.userId._id} className="bg-black/40 backdrop-blur-xl border border-white/10 shadow-xl rounded-2xl p-5 relative overflow-hidden">
                ...
                <div className={`absolute top-0 right-0 w-32 h-32 rounded-full blur-3xl opacity-20 -mr-10 -mt-10 pointer-events-none transition-colors ${
                  ...
                  isHalfDone ? 'bg-amber-500' : 'bg-red-500'
                }`} />

                <div className="flex justify-between items-start mb-3 relative z-10">
                  <div>
                    <h3 className="font-bold text-white text-lg flex items-center gap-2">
                      {member.userId.displayName} {isSelf && <span className="text-[10px] bg-white/20 text-white font-bold px-2 py-0.5 rounded-full">YOU</span>}
                    </h3>
                    <p className="text-xs text-white/50 mt-1 uppercase tracking-widest font-semibold flex items-center gap-1.5">
                      <span className={`h-1.5 w-1.5 rounded-full ${
                        isFullyCompleted ? 'bg-emerald-400' :
                        isHalfDone ? 'bg-amber-400' : 'bg-red-400'
                      }`} />
                      {isFullyCompleted ? 'COMPLETED' : isHalfDone ? 'ON TRACK' : 'AT RISK'}
                    </p>
                  </div>
                ...
                <div className="mb-4 relative z-10">
                  <div className="flex justify-between text-xs text-white/60 mb-2 font-semibold">
                    ...
                    <span>{member.todayCompleted} / {memberTasksCount}</span>
                  </div>
                  <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden border border-white/5">
                    <div 
                      className={`h-full rounded-full transition-all duration-1000 ${
                        isFullyCompleted ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]' :
                        isHalfDone ? 'bg-amber-500' : 'bg-indigo-500'
                      }`}
                      style={{ width: `${(member.todayCompleted / memberTasksCount) * 100}%` }}
                    />
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-white/10 relative z-10">
                  <button
                    ...
                    className="flex-1 bg-white/10 hover:bg-white/20 text-white font-bold text-xs py-2 px-3 rounded-xl border border-white/10 transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                  >
                    <Eye className="w-3.5 h-3.5 text-white" /> Inspect Checklist
                  </button>
                  ...
                  {!isSelf && (
                    <>
                      <button
                        ...
                        className="bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 font-bold text-xs py-2 px-3 rounded-xl border border-amber-500/30 transition-all flex items-center gap-1 cursor-pointer"
                      ...
                      <button
                        ...
                        className="bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-300 font-bold text-xs py-2 px-3 rounded-xl border border-indigo-500/30 transition-all flex items-center gap-1 cursor-pointer"
